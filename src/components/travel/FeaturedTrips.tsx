    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {list.map((d, i) => (
        <motion.article
          ...
          whileHover={{ y: -8, boxShadow: '0 16px 50px rgba(245,158,11,0.18)' }}
          className="glass-panel overflow-hidden img-zoom group"
        >
          <div className="relative h-44">
            <CoverImage src={d.cover_image_url} alt={d.title} className="w-full h-full" fallbackGlyph="🏔" />
            <span className="absolute top-3 left-3 text-[10px] font-mono uppercase tracking-widest bg-void/70 backdrop-blur-sm text-ember border border-ember/30 rounded-full px-2.5 py-1">
              {d.country}
            </span>
          </div>
          <div className="p-5">
            <h3 className="font-display text-lg text-mist group-hover:text-ember transition-colors">{d.title}</h3>
