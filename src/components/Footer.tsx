...
export default function Footer({ profile }: { profile: Profile }) {
  ...
  return (
    <footer className="relative border-t border-gold/10 bg-charcoal/60">
      <div className="max-w-6xl mx-auto px-5 sm:px-8 py-10 flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="text-center sm:text-left">
          <p className="font-display tracking-widest text-mist">
            VIVEK RANJAN<span className="text-gold">.</span>
          </p>
        ...
        <nav aria-label="Social links" className="flex items-center gap-5">
          ...
          <a href={profile.github_url} target="_blank" rel="noopener noreferrer" className="text-ash hover:text-gold-light transition-colors" aria-label="GitHub">
          ...
          <a href={`mailto:${profile.email}`} className="text-ash hover:text-gold-light transition-colors" aria-label="Email">
        ...
        <button
          ...
          className="btn-frost !px-4 !py-2 text-sm"
